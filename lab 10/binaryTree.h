...
public:
    ...
    int treeLeavesCount();
      ...
      //               is returned.
    int singleParent(); 
     //function to determine the number f single parents
     //postcondion: number of single parent in the binary tree is returned
...
private:
    ...
    int singleParentCount(nodeType<elemType> *p);  
      //function to determine the number of single parents in the binary tree
      //postcondition: returns the number of parent that in the tree pointed by pointer p
};
...
int binaryTreeType<elemType>::leavesCount(nodeType<elemType> *p)
{
	...
	if(p->rlink==NULL && p->llink==NULL)
	  return 1;
       ...
       count=count+leavesCount(p->llink);
    else if(p->llink==NULL)
       count=count+leavesCount(p->rlink);   
...
template<class elemType>
int binaryTreeType<elemType>::singleParentCount(nodeType<elemType> *p)
{
	int count=0;
	//cout<<count<<" intial.. "<<endl;
	if(p->rlink==NULL && p->llink!=NULL)
	  ...
	  return 0;    
	//else if(p->rlink==NULL)
       //count=count+singleParentCount(p->llink);
    //else if(p->llink==NULL)
      // count=count+singleParentCount(p->rlink);   
