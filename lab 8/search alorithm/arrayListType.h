   ...
   void insertEnd(const elemType& insertItem);
    ...
    int seqSearch(const elemType& item);
		...
		//               in the array where the item is found; 
   		//               otherwise, returns -1.
   	void newSeqSearch(const elemType& item);	
...
template <class elemType>
void arrayListType<elemType>::newSeqSearch(const elemType& item)
{
	int loc, count=0;
	bool found = false;
	...
	if(found)
   	   cout<<"SeqSearch    : "<<item<<" found at location "<<loc<<". Steps = "<<count<<endl;
	else
		cout<<"SeqSearch    : "<<item<<" could not be found. Steps = "<<length<<endl;;
} //end seqSearch
template <class elemType>
